...
class Piece {
  ...
  userMove(e) {
    if ([37, 39, 40].includes(e.keyCode)) {
      if (__WEBPACK_IMPORTED_MODULE_0__util__["e" /* shouldPieceBeMovedByUser */](e, this.piece)) {
        this.removeCurrentPiece();
        const piece = __WEBPACK_IMPORTED_MODULE_0__util__["f" /* movePieceByUser */](e, this.piece);
        this.piece = piece;
      }
    }
  }
...
const movePieceByUser = (e, piece) => {
  const collection = generateUserMoveCollection(e, piece);
...
const generateUserMoveCollection = (e, piece) => {
  const delta = generateMoveDelta(e.keyCode);
  let collection = $d();
  let newSqPos;
  let newSq;
  let newYpos;
...
const shouldPieceBeMovedByUser = (e, piece) => {
  const delta = generateMoveDelta(e.keyCode);
  let collection = $d();
  let newYpos;
  let newXpos;

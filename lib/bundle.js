...
class Piece {
  ...
  rotatePiece() {
    if (this.isPaused()) {
      this.removeCurrentPiece();
      const rotatedPiece = __WEBPACK_IMPORTED_MODULE_0__util__["e" /* rotatePiece */](this.piece, this.orientation);
  ...
  userMovePiece(key) {
    if (this.isPaused()) {
      if (__WEBPACK_IMPORTED_MODULE_0__util__["f" /* shouldPieceBeMovedByUser */](key, this.piece, this.orientation)) {
        this.removeCurrentPiece();
        const piece = __WEBPACK_IMPORTED_MODULE_0__util__["g" /* movePieceByUser */](key, this.piece);
        this.piece = piece;
      }
    }
  }
...
const pieceDeltas = {
  ...
  // 0, 0: horizontal, 0, 1: vertical
  //
***REMOVED***

const generateRotateDeltas = shapeId => {
  return rotateDeltas[shapeId];
***REMOVED***
...
const rotatePiece = (piece, orientation) => {
  const { startXpos, shapeId, yPos } = getAttributes(piece);
  const deltas = generateRotateDeltas(shapeId);
  const collection = generateCollection(startXpos, yPos, shapeId, deltas);
  return finalizePiece(collection, shapeId);
***REMOVED***
/* harmony export (immutable) */ exports["e"] = rotatePiece;
...
const shouldPieceBeMovedByUser = (key, piece, orientation) => {
  return isPieceInBounds(key, piece) &&
  shouldPieceMoveAgain(piece, orientation);
***REMOVED***
/* harmony export (immutable) */ exports["f"] = shouldPieceBeMovedByUser;


const isPieceInBounds = (key, piece) => {
  const delta = generateMoveDelta(key);
  let collection = $d();
  let newXpos;
  let HTMLel;
...
const shouldPieceMoveAgain = (piece, orientation) => {
  ...
  } else if (shapeId === 0 && orientation === 1) {
    if (collisionCount > 3) {
      return false;
    }
  } else {
